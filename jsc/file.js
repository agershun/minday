...
var cloneIdea, fileCollaborative, fileExit, fileExport, fileExportODP, fileExportODT, fileImport, fileNew, fileOpen, fileSave, fileSaveAs, fileStart, filename, importFile, setMenuFile;

filename = "minday";

setMenuFile = function() {
  ...
  $(".file_exportodp").click(fileExportODP);
  $(".file_exit").click(fileExit);
  return $(".file_saveasserver").click(fileSaveAs);
};

fileStart = function() {
  return selectModel("start", true);
};
...
cloneIdea = function(source) {
  var i, ia, ik, key, _i, _len, _ref;
  ia = idea[-source.id];
  if (source.data != null) {
    _ref = source.data;
  ...
  for (key in source) {
    if (source.hasOwnProperty(key) && key !== "data") {
      ik = source[key];
      if (key !== "id" && key !== "list" && key !== "kind" && key !== "ix" && key !== "length") {
        if (typeof ik === "number") {
          ik = idea[Math.abs(ik)];
        }
      }
...
fileOpen = function() {
  ...
  return idea[-b.sideaid].upperFrame().fhtml();
};

fileCollaborative = function() {
...
Idea.prototype.toJSON = function() {
  ...
  for (key in this) {
    if (this.hasOwnProperty(key) && typeof key === "string" && key !== "begetObject") {
      val = this[key];
      if (isNaN(key)) {
        ...
        } else if (typeof val === "object" && val instanceof Idea) {
          ret[key] = val.id;
        }
      }
    }
...
fileSave = function() {
  ...
  return alert("Файл сохранен в локальное хранилище");
};
...
fileExportODT = function() {
  var ia, str, zip, _i, _len;
  zip = new JSZip();
  zip.folder("META-INF");
  ...
  for (_i = 0, _len = fidea.length; _i < _len; _i++) {
    ia = fidea[_i];
  ...
  return location.href = "data:application/vnd.oasis.opendocument.text;base64," + zip.generate();
};

fileExportODP = function() {
  return alert("Функция экспорта в PowerPoint временно отключена, из-за большого формата файла");
};
